...
export default function PartnerPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (
      ...
      setError(
        "Please complete the required fields so we can review your application.",
      );
      return;
    }
    createPartner.mutate(
      {
        ...
        suburbs: form.suburbs
          ...
          .map((item) => item.trim())
          .filter(Boolean),
        radiusKm: Number(form.radiusKm) || 15,
        emergencyJobs: form.emergencyJobs,
      ...
      <header className="border-b border-[hsl(var(--border))]">
        <div className="content-wrap flex min-h-[76px] items-center justify-between">
          ...
          <Link
            ...
            data-testid="link-partner-home"
          >
            Back home
          </Link>
        </div>
      </header>
      <main className="content-wrap grid max-w-[1100px] gap-10 py-10 pb-20 md:grid-cols-[.7fr_1.3fr] md:py-16">
        <div className="md:sticky md:top-10 md:self-start">
          ...
          <h1 className="mt-3 text-5xl font-bold leading-[.9] tracking-[-.075em] md:text-7xl">
            Work that
            <br />
            <span className="font-display font-normal italic text-[hsl(var(--secondary))]">
              fits.
            </span>
          </h1>
          <p className="mt-7 max-w-sm text-base leading-7 text-[hsl(var(--muted-foreground))]">
            SourceTradie brings the right local jobs to good operators. You
            decide when you’re available and what’s worth your time.
          </p>
          <div className="mt-10 space-y-4">
            <Promise
              icon={<MapPin size={17} />}
              title="Stay in your patch"
              detail="Set your suburbs and travel radius."
            />
            <Promise
              icon={<ShieldCheck size={17} />}
              title="Better briefings"
              detail="See the customer’s real words before deciding."
            />
            <Promise
              icon={<FileCheck2 size={17} />}
              title="No lock-in"
              detail="Apply once. Keep control of your diary."
            />
          </div>
        </div>
        <form
          onSubmit={submit}
          className="glass-card rounded-[1.5rem] p-5 md:p-8"
        >
          <div className="flex items-center justify-between border-b border-[hsl(var(--border))] pb-5">
            <div>
              <SectionLabel>Expression of interest</SectionLabel>
              <h2 className="mt-2 text-2xl font-bold tracking-[-.05em]">
          ...
          <div className="mt-7 space-y-6">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <Field
                ...
                placeholder="hello@northsideflow.com.au"
                type="email"
              />
            </div>
            <div className="grid gap-5 sm:grid-cols-[1.4fr_.6fr]">
              <Field
                ...
                placeholder="Brunswick, Coburg, Preston"
              />
              <Field
                label="Radius (km)"
                id="radiusKm"
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <Field
                ...
                id="licence"
                value={form.licence}
                onChange={(value) => update("licence", value)}
                placeholder="REC / VBA number"
              />
              <Field
                label="Services"
                id="services"
                ...
                placeholder="Leaks, hot water, repairs"
              />
            </div>
            <label className="flex cursor-pointer items-start gap-3 rounded-xl bg-[hsl(var(--muted)/.65)] p-4 text-sm leading-6">
              <input
                type="checkbox"
                checked={form.emergencyJobs}
                onChange={(event) =>
                  update("emergencyJobs", event.target.checked)
                }
                className="mt-1 h-4 w-4 accent-[hsl(var(--secondary))]"
                data-testid="checkbox-emergency-jobs"
              />
              <span>
                <strong>Open to emergency jobs</strong>
                <br />
                <span className="text-[hsl(var(--muted-foreground))]">
                  Only choose this if you can safely respond outside regular
                  hours.
                </span>
              </span>
            </label>
          ...
          <div className="mt-7 flex flex-col gap-3 border-t border-[hsl(var(--border))] pt-6 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-[hsl(var(--muted-foreground))]">
              We’ll review your details before approval.
            </p>
            <button
...
function Promise({
  ...
  return (
    <div className="flex gap-3">
      <span className="mt-0.5 text-[hsl(var(--secondary))]">{icon}</span>
      <div>
      ...
      <main className="content-wrap grid min-h-[calc(100dvh-77px)] max-w-[720px] place-items-center py-16 text-center">
        <div className="animate-rise">
          ...
          <p className="mx-auto mt-7 max-w-md text-base leading-7 text-[hsl(var(--muted-foreground))]">
            Your application has been safely received and is awaiting review.
            SourceTradie will contact you using the business details you
